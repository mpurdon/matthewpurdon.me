...
It was a real bug. The image had been told it could be as tall as the box it sits in. That box had been told it could be as tall as the box *it* sits in, and so on up to the page itself, so at no point had anybody stated an actual number. Chrome is willing to work its way back down that chain and invent one. Safari waits to be told, and when nobody tells it, hands back less. The agent worked this out faster than I would have, replaced the chain with arithmetic (window height, minus the footer, minus the page margins), and checked for overflow at 1440x900, 1280x620 and 390x844. Its reply: "Your instinct was right, that wasn't a feel difference, it was a real bug in my CSS."
...
The real cause was a layer further down. The page ships the picture at several different resolutions and lets the browser pick whichever suits the space. Because the styling never gave the image a width of its own, the size of whichever copy got picked became the size of the picture on the page. Shrink the window, the browser picks a smaller copy, and the picture shrinks. Grow it back and Chrome does the sum again while Safari keeps the smaller answer it already has. So every shrink lowered the ceiling a notch and nothing raised it again, and reloading started the calculation over, which is why the image jumped back up.
...
The agent out-diagnosed me twice and would have kept going all day. What it could not do was look at the page and be bothered. For nine turns, neither could I.

## Three minutes is not a good time

That is a toy. One image on one page, no users, no money. It stuck with me because the same shape turned up a week later at production scale with money attached.
...
CORA taught me the other half too. It orders my pull request queue using weights I set on the repository and on the person who opened the change, so an AWS SSO configuration change from the engineer who always draws the hairiest problems on the tightest deadlines sits well above a maintenance version bump in a repository I barely touch, opened by a junior. None of that came from the model. Asked for a list, it sorted alphabetically. Asked to think about the problem I actually had (there are 250 pull requests open, how do I know which one deserves me right now), it came back with a file-importance weighting, which was good enough that I kept it, and that was as far as it went. It was never going to work out that I care about some repositories more than others, or that certain people get handed the work most likely to hurt. How would it know? Its world is a small place, and its window into that world is smaller.

The blank spot shows up from the other direction too. Nothing suggested that meme to me. I had wanted it for twenty years, the thing that could finally make it had been sitting on my desktop for weeks, and at no point did it say "that thing you have been waiting on, I can do that now". I had to go and wonder. Claude has watched me push dozens of enormous pull requests through it and ask for a summary as a principal engineer, and it knew I had spent the spring contributing to Marrow, and it has never once suggested I build my own reviewer. I built CORA anyway. Every question these tools have answered well for me is one I brought them.
...
Here's the thing nobody wants to say out loud in the AI argument: the code review was already broken before the robots showed up. Fifteen years of LGTM and a thumbs-up emoji on a forty file PR. "can you approve this real quick, it's blocking the release" ... and we all did it, and then we all wrote the review step up in the postmortem like it was a control that existed. brutal.
...
I'm not saying throw it out. I'm saying stop citing it. If your team's average is three minutes, you have two options and only two: make the review real, which costs time you will have to actually take from somewhere, or admit it isn't one and go spend that effort where it lands (design conversations, tests that mean something, a limit that makes invisible costs visible before the quota does it for you). What you can't keep doing is running a ceremony and calling it a gate. Everyone knows. We've all known for years.
