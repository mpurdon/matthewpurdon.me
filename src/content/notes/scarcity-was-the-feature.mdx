...
<Callout variant="takeaway" title="The glance">
  Metered compute once forced engineers to think before they ran anything, and to invent time-sharing, virtualization, and most of what became the cloud. Abundance erased the habit; token pricing has reinstated it. Canada's sovereign AI conversation treats our inability to out-spend the hyperscalers as the fatal flaw, which is the wrong thing to be embarrassed about. A national platform should be a registry of governed, evaluated capabilities rather than a model zoo, with metered compute doing what it did in the mainframe era. The same meter pays for an API key for every Canadian and a marketplace where anyone can publish a capability. It should work like a library card.
</Callout>
