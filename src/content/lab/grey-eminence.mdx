...
I assumed I needed one good transcription model. I actually needed two flawed ones, flawed in opposite directions. During the meeting, latency wins: a small streaming model keeps the words close enough to real time to read along, and you forgive its sins because they scroll past. After the meeting, accuracy wins: the recording goes back through a model thirty times the size, and that version replaces the live one as the document of record.
